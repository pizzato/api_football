...
def fetch_and_dump():
    api = APIFootballExtended(key=KEY, host=HOST)

    file_append_str = '{league}_{season}_{now}'.format(league=LEAGUE, season=SEASON, now=now)
    os.makedirs(FOLDER)

    print("Getting Teams")
    teams_dict = api.get_map_team_name_id(league=LEAGUE, season=SEASON)
    ...
    players_df = api.get_league_players(teams=teams_dict.values())
    ...
    current_round = api.get_current_round(league=LEAGUE, season=SEASON)
